...
export default function FreeDiaryForm({ diary, date }: FreeDiaryFormProps) {
  ...
  const handleSave = async () => {
    ...
    try {
      if (diary) {
        await updateFreeDiary(diary.id, content);
      } else {
        await createFreeDiary(content, date);
      }
      router.push("/");
    } catch (e) {
        ...
        <main className="flex-1 overflow-y-auto px-5 pb-24 hide-scrollbar">
          <div className="rounded-2xl border border-border bg-card p-4 shadow-sm">
            <textarea
              ...
              autoFocus
              className="w-full min-h-[400px] bg-transparent text-[15px] leading-7 text-foreground placeholder:text-muted/60"
            />
          </div>
        </main>
